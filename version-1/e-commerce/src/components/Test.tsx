...
const ModernEcommerceHomepage = () => {
  ...
  // Fetch featured products (one per category, show all categories)
  const fetchFeaturedProducts = async () => {
    try {
      ...
      const featured = Object.values(productsByCategory).map(
  ...
  const fetchTopRatedProducts = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/public/products?limit=500`
      );
  ...
  const getCategoryOrSubcategoryImage = (categoryId: number) => {
    ...
    let productsInGroup = allProducts.filter(
      (p) => p.category_id === categoryId
    );
    console.log("Found products for category:", productsInGroup);
    if (productsInGroup.length > 0) {
      ...
      if (top && top.images && top.images.length > 0) {
        ...
        console.log("No images found for top product:", top);
      }
    } else {
